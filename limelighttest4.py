...
def runPipeline(frame, llrobot):
    ...
    def ransac(
        ...
        for _ in range(max_iterations):
            ...
            result = create_circles(p1, p2, p3)
    ...
    def circle_support(points, circle, threshold=4.0):
        cx, cy, r = circle
    ...
    def findpollen(points, max_circles=10):
        ...
        for _ in range(max_circles):
            if len(remaining) < 20 or cv.contourArea(cnt) < 100:
                break
            ...
            is_inlier = np.abs(distances - r) < 7.0
            remaining = remaining[~is_inlier]
    ...
    return np.array([]), img, [0, 0, 0, 0, 0, 0, 0, 0]